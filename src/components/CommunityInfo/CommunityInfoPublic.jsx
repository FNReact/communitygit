...
const CommunityInfoPublic = ({ msPublicDetails, memberStatus }) => {
  ...
  const [details, setDetails] = useState(
    msPublicDetails?.entity_details ? msPublicDetails.entity_details : ""
  );
  ...
  return (
    <Fragment>
      <div className="communityInfo_wrapper mob_padding_75 communityInfo_wrapper_small">
        {msPublicDetails?.meta?.banner && (
          <div className="commuinity_banner">
            <img
              src={`${baseUrl}/${msPublicDetails?.meta?.banner}`}
              alt={msPublicDetails?.name}
            />
          </div>)}
        <div className="community_top_info">
          <div className="com_one">
            <div className="com_logo">
              <img
                src={`${baseUrl}/${msPublicDetails?.entity_logo}`}
                alt={msPublicDetails?.name}
              />
            </div>
            <div className="com_top_left">
              ...
              <div className="tagline">
                {msPublicDetails?.tagline
                  ? parse(msPublicDetails?.tagline)
                  : ""}{" "}
              </div>
            </div>
          </div>
          ...
          {token && loggedMemberStatus === 0 && (
            <Alert severity="info">
              Your request has been sent to admin for aproval. Your joining
              request is pending...
            </Alert>
          )}
          {token && loggedMemberStatus === 2 && (
            <Alert severity="info">
              You have invited to join this community.
            </Alert>
          )}
          ...
          {token && loggedMemberStatus === null && (
            <div className="community_view_btn" onClick={(e) => handlejoin()}>
              {" "}
              Join{" "}
            </div>
          )}
          ...
          {token && loggedMemberStatus === 1 && (
            <div className="community_view_btn" onClick={(e) => handleLounge()}>
              {" "}
              Visit Lounge{" "}
            </div>
          )}
          ...
          <Grid container spacing={2}>
              ...
              <div className="com_info_wrap">
                {msPublicDetails?.entity_email && (
                  <div className="info_item">
                    {" "}
                    E-mail : <span> {msPublicDetails?.entity_email} </span>
                  </div>
                )}
                {msPublicDetails?.entity_phone && (
                  <div className="info_item">
                    {" "}
                    Phone : <span> {msPublicDetails?.entity_phone}</span>
                  </div>
                )}
                {msPublicDetails?.location && (
                  <div className="info_item">
                    {" "}
                    Location : <span> {msPublicDetails?.location}</span>
                  </div>
                )}
                {msPublicDetails?.address && (
                  <div className="info_item">
                    {" "}
                    Address : <span> {msPublicDetails?.address}</span>
                  </div>
                )}
                {msPublicDetails?.website &&
                  msPublicDetails?.website !== "null" && (
                    <div className="info_item">
                      {" "}
                      Website : <span> {msPublicDetails?.website}</span>
                    </div>
                  )}
              </div>
            </Grid>
            <Grid item xs={4}>
              <div className="qr_img">
                <img
                  src={`${baseUrl}/storage/images/microsites/qr/${msPublicDetails.id}.png`}
                  alt=""
                />
              </div>
            </Grid>
