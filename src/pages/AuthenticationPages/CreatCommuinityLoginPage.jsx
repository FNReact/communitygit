...
const CreatCommuinityLoginPage = () => {
    ...
    if(email && password){
      ...
      .then(function (response) {
          if (response.data.token) {
            ...
            window.location.href='/community-packeges'
              ...
              {(location.state !== null && location.state.message===true ) && <button className="btn btn-warning" disabled>Your account is not yet activated.An activation link was sent to your email. Active first to create your microsite.</button>}
                    <form>
                        <Grid container spacing={1} sx={{mt:2}}>
                            <Grid item xs={12}>
                              <input type="email" disabled className="form_control" onChange={(e)=> setEmail(e.target.value)} value={email}  placeholder="Enter Your E-mail" />
                            </Grid>
                            <Grid item xs={12}>
...
export default CreatCommuinityLoginPage;
